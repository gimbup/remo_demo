...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if($_POST["action"] == "Delete"){
        deleteInstructor($_POST["lastname"], $_POST["firstname"], $_POST["email"]);
    }
}
...
function createInstructor(){
    ...
    if($flag == true){
        ...
        $query = "SELECT * FROM Instructor WHERE firstname='$firstname' AND lastname='$lastname' AND email='$email' AND active=1";
...
function deleteInstructor($lastname, $firstname, $email){
    $firstname = mysql_real_escape_string($firstname);
    $lastname = mysql_real_escape_string($lastname);
    $email = mysql_real_escape_string($email);

    $query = "DELETE FROM Instructor Where firstname='$firstname' AND lastname='$lastname' AND email='$email'";
...
<table class="list">
  ...
  <?php
  $query = "SELECT lastname, firstname, email, position FROM Instructor WHERE active = 1 ORDER BY lastname, firstname ASC";
  $result = mysql_query($query);
  ...
  if (mysql_num_rows($result) > 0){
    ...
    while($row = mysql_fetch_assoc($result)){
        ...
        <td class=row>". $row["lastname"]. ", " . $row["firstname"] . "</td>
